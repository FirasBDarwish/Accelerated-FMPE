...
class Base:
    ...
    def network_to_device(self, device):
        ...
        self.device = dev                 # keep attribute in sync FIRST
    ...
    def load_model(
        ...
        """
        Load a posterior model from disk (CPU-safe), then move to the requested device.
        """
        import os
        import torch
        ...
        if dev_str.startswith("cuda") and not torch.cuda.is_available():
            dev_str = "cpu"
        if dev_str == "cpu":
            # Optional: prevent accidental CUDA probes in submodules
            os.environ.setdefault("CUDA_VISIBLE_DEVICES", "")
        self.device = torch.device(dev_str)  # <<< set BEFORE building network
    
        # ---------- 2) Always load checkpoint on CPU ----------
        d = torch.load(model_filename, map_location="cpu")
    
        self.version = d.get("version")
        self.model_kwargs = d["model_kwargs"]
    
        # ---------- 3) Build the network on the chosen device ----------
        self.initialize_network()  # must respect self.device internally
    
        # ---------- 4) Load state_dict (handle DP prefix) ----------
        state_dict = d["model_state_dict"]
        is_wrapped = isinstance(self.network, nn.DataParallel)
        ...
        target = self.network.module if is_wrapped else self.network
        ...
        # ---------- 5) Unwrap DP on CPU to avoid CUDA side-effects ----------
        if dev_str == "cpu" and isinstance(self.network, nn.DataParallel):
            self.network = self.network.module
    
        # ---------- 6) Move using the fixed helper (keeps self.device in sync) ----------
        self.network_to_device(dev_str)
