...
class ContinuousFlowsBase(Base):
    ...
    def __init__(self, **kwargs):
        ...
        self.device = self.metadata["train_settings"]["training"]["device"]
    ...
    def initialize_network(self):
        # Build the model from kwargs (unchanged)
        ...
        # --- Resolve and enforce device safely ---
        import torch
        # Normalize device; default to CPU if unset
        dev = getattr(self, "device", None)
        if dev is None:
            dev = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        elif isinstance(dev, str):
            # if 'cuda' was passed but not usable, fall back to cpu
        ...
        # Move the freshly-built network to the chosen device
        self.network = self.network.to(self.device)
    
        # --- DataParallel only when actually on CUDA with multiple GPUs ---
        # IMPORTANT: check is_available() BEFORE device_count()
        if self.device.type == "cuda" and torch.cuda.is_available():
            n_gpus = torch.cuda.device_count()
            if n_gpus > 1:
                try:
                    eprint(f"✅ Using {n_gpus} GPUs with DataParallel")
                except NameError:
